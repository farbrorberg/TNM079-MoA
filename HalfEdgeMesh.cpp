...
bool HalfEdgeMesh::AddFace(const std::vector<Vector3<float> > &verts){
  ...
  unsigned int inner1, outer1, inner2, outer2, inner3, outer3;
  AddHalfEdgePair(ind1, ind2, inner1, outer1);
  AddHalfEdgePair(ind2, ind3, inner2, outer2);
  AddHalfEdgePair(ind3, ind1, inner3, outer3);

  mEdges[inner1].pair = outer1;
  mEdges[inner2].pair = outer2;
  mEdges[inner3].pair = outer3;

  // Connect inner ring. 
  mEdges[inner1].next = inner2;
  mEdges[inner1].prev = inner3;

  mEdges[inner2].next = inner3;
  mEdges[inner2].prev = inner1;

  mEdges[inner3].next = inner1;
  mEdges[inner3].prev = inner2;

  // Finally, create the face, don't forget to set the normal (which should be normalized)
  Face face;
  face.edge = inner1;
  mFaces.push_back(face);
  mFaces.back().normal = FaceNormal(mFaces.size() - 1);	//Normalize the face normal at the "end" of the stack: n-1

  mEdges[inner1].face = mFaces.size() - 1;
  mEdges[inner2].face = mFaces.size() - 1;
  mEdges[inner3].face = mFaces.size() - 1;
...
std::vector<unsigned int> HalfEdgeMesh::FindNeighborVertices(unsigned int vertexIndex) const
{
  ...
  HalfEdge starEdge = mEdges[mVerts[vertexIndex].edge];

  // Add your code here
  starEdge = mEdges[starEdge.next];
  oneRing.push_back(starEdge.vert);

  HalfEdge itr = mEdges[starEdge.next];
  
  while (itr.vert != starEdge.vert) {
	  itr = mEdges[itr.pair];
	  itr = mEdges[itr.next];
	  oneRing.push_back(itr.vert);
...
std::vector<unsigned int> HalfEdgeMesh::FindNeighborFaces(unsigned int vertexIndex) const
{
  ...
  HalfEdge startEdge = mEdges[mVerts[vertexIndex].edge];
  foundFaces.push_back(startEdge.face);
  
  HalfEdge itr = mEdges[startEdge.prev];
  itr = mEdges[itr.pair];
  while (itr.vert != startEdge.vert) {
	  foundFaces.push_back(itr.face);
...
Vector3<float> HalfEdgeMesh::VertexNormal(unsigned int vertexIndex) const
{
  ...
  faces = FindNeighborFaces(vertexIndex);
  
  for (int i = 0; i < faces.size(); i++) {
	  n += mFaces[faces[i]].normal;
  }
  // Add your code here
  return n.Normalize();
}
...
float HalfEdgeMesh::Area() const
{
  ...
  for (int i = 0; i < mFaces.size(); i++) {
	  ...
	  area += Cross(mVerts[mEdges[halfEdge.next].vert].pos - mVerts[halfEdge.vert].pos, mVerts[mEdges[halfEdge.prev].vert].pos - mVerts[halfEdge.vert].pos).Length()*0.5;
  }
  // Add code here
  return area;
}
